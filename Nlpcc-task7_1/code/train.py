...
def train(args, tokenizer, model):
    ...
    no_decay = ['bias', 'LayerNorm.weight']#81-86行的代码作用就是在该列表中的参数不参与权重衰减
    ...
    args.warmup_steps = int(t_total * args.warmup_rate)#学习率有关的耐心系数，可以动态调整学习率
    ...
    logger.info("  Gradient Accumulation steps = %d", args.gradient_accumulation_steps)#解决显存不足的情况
    ...
    best_acc = 0
    model.zero_grad()
    train_iterator = trange(int(args.num_train_epochs), desc="Epoch", disable=args.local_rank not in [-1, 0])#就是一个进度条
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            loss1 = loss_fun1(output1,label1)
            loss2 = loss_fun2(output2,label2)
            loss = loss1 + loss2
...
def evaluate(args, model, eval_dataset=None, tokenizer=None):
    ...
    args.eval_batch_size = 1        #我把验证集全加进去试试效果
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            output1,output2 = model(inputs)
            tmp_eval_loss = loss_fun(output1,label1)
            eval_loss += tmp_eval_loss.mean().item()
        nb_eval_steps += 1
        if preds is None:
            preds = output1.detach().cpu().numpy()
            out_label_ids = inputs["labels"].detach().cpu().numpy()
        else:
            preds = np.append(preds, output1.detach().cpu().numpy(), axis=0)
...
def main():
    ...
    parser.add_argument(
        "--model_name_or_path",
        default='Langboat/mengzi-bert-base',
        type=str,
    ...
    if args.do_train:
        ...
        model = BertForSequence2(model,config)
